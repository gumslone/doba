...
> and an interim admin with a WYSIWYG editor. Still to come: the guest
> checkout funnel, channel/OTA sync, the installation wizard, the public partner API and the full
> Filament panel — all specified in
> [`docs/architecture.md`](docs/architecture.md). See [Roadmap](#roadmap).
> The correctness-critical paths are covered by 155+ tests on both database
