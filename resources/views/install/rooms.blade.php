        ...
        <div>
            ...
            <div class="mt-2 space-y-2">
                @foreach (range(0, 4) as $i)
                    <div class="grid gap-2 sm:grid-cols-[2fr_1fr_1fr_1fr_1fr]">
                        ...
                        <input name="rooms[{{ $i }}][price]" type="number" step="0.01" min="0" placeholder="{{ __('install.room_price') }}"
                               ...
                               class="rounded border border-neutral-300 px-3 py-2 text-sm">
                        <select name="rooms[{{ $i }}][kind]" aria-label="{{ __('install.room_kind') }}" class="rounded border border-neutral-300 px-3 py-2 text-sm">
                            @foreach (\App\Models\RoomType::KINDS as $kind)
                                <option value="{{ $kind }}" @selected(old('rooms.'.$i.'.kind') === $kind)>{{ __('install.room_kind_'.$kind) }}</option>
                            @endforeach
                        </select>
                    </div>
                @endforeach
